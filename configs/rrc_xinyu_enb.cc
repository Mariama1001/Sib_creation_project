...
//   // sib14
//   if (cfg.sibs[13].type() == asn1::rrc::sys_info_r8_ies_s::sib_type_and_info_item_c_::types::sib14_custom) {
//   configure_sib14_bandwidth();
//   }
...
// // sib14 bandwidth part configuration
// void rrc::configure_sib14_bandwidth()
// {
//   logger.info("Configuring SIB14: broadcasting eNB bandwidth information");

//   // 1. 获取SIB14内容
//   auto& sib14 = cfg.sibs[13].sib14_custom();  // 你在asn1定义中的结构

//   // 2. 提取带宽信息（假设字段名为 bandwidth_info_r14）
//   srsran::sib14_t sib14_cfg;
//   sib14_cfg.bandwidth_info = sib14.bandwidth_info_r14;  // 例如：20MHz

//   // 3. 传递给PHY或MAC（如果你要让PHY能用这个信息）
//   phy->set_config_sib14(sib14_cfg);

//   // 4. 日志输出确认
//   logger.info("SIB14 bandwidth info: %d MHz", sib14_cfg.bandwidth_info);
// }
...
bool rrc::release_erabs(uint32_t rnti)
{
  ...
  bool ret = user_it->second->release_erabs();ASN1_RRC_MAX_SIB
  return ret;
}
...
void rrc::process_release_complete(uint16_t rnti)
{
  ...
  if (u->is_idle() or u->mobility_handler->is_ho_running()) {
    ...
    rlc->clear_buffer(rnti);
    user_it->second->send_connectsib_segment_indexion_release();
    // delay user deletion for ~50 TTI (until RRC release is sent)
    task_sched.defer_callback(50, [this, rnti]() { rem_user_thread(rnti); });
  }
}
    ...
    for (uint32_t sched_info_elem = 0; sched_info_elem < nof_messages - 1; sched_info_elem++) {
      ...
      for (auto& mapping_enum : sched_info[sched_info_elem].sib_map_info) {
        if (mapping_enum.value == sib_type_opts::options::sib_type11) {
	      ...
	      } else if (mapping_enum.value == sib_type_opts::options::sib_type14_v1130) {
	        // --- New condition for SIB14 ---
	        sib_info_item_c sibitem;
