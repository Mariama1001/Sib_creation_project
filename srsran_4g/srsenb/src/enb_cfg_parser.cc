...
// Define SIB14 fields
int parse_sib14(std::string filename, sib_type14_r11_s* data)
{
...
int parse_sibs(all_args_t* args_, rrc_cfg_t* rrc_cfg_, srsenb::phy_cfg_t* phy_config_common)
{
  ...
  if (sib_is_present(sib1->sched_info_list, sib_type_e::sib_type14_v1130)) {
...
//             data->sched_info_list[i].sib_map_info[j].value = (sib_type_e::options)(sib_index - 3);
//       //******************************************************************************************************** */
//           } else if (sib_index == 24) {
//             // 这里依赖你在 ASN.1/rrc 枚举里新增的 sib_type24（名字以你生成代码为准）
//             data->sched_info_list[i].sib_map_info[j] = sib_type_e::sib_type24_v1530;
//       //******************************************************************************************************** */
//           } else {
//             fprintf(stderr, "Invalid SIB index %d for si_mapping_info=%d in sched_info=%d\n", sib_index, j, i);
//             return SRSRAN_ERROR;
...
// //**************************************************************************************************** */
// int parse_sib24(std::string filename, asn1::rrc::sib_type24_r15_s* data)
// {
//   parser::section sib24("sib24");

//   // Create a subsection for CarrierFreqListNR-r15
//   parser::section carrier_freq_list_nr("carrier_freq_list_nr");
//   sib24.add_subsection(&carrier_freq_list_nr);

//   // Add fields similar to other SIBs
//   carrier_freq_list_nr.add_field(
//       make_asn1_seqof_size_parser("carrier_freq_nr_r15_size", &data->carrier_freq_list_nr_r15));

//   // Add any additional fields specific to SIB24-r15 structure
//   // Example (adjust based on actual ASN.1 structure):
//   sib24.add_field(new parser::field<uint32>("t_reselection_nr_r15", &data->t_reselection_nr_r15));

//   return parser::parse_section(std::move(filename), &sib24);
// }
// //*************************************************************************************************** */
...
//   //**************************************************************************************************** */
//   asn1::rrc::sib_type24_r15_s* sib24 = &rrc_cfg_->sibs[23].set_sib24_r15();
//  //**************************************************************************************************** */
...
//   }
// //**************************************************************************************************** */
//     // Generate SIB24 if defined in mapping info
//   if (sib_is_present(sib1->sched_info_list, sib_type_e::sib_type24_v1530)) {
//     if (sib_sections::parse_sib24(args_->enb_files.sib_config, sib24) != SRSRAN_SUCCESS) {
//       return SRSRAN_ERROR;
//     }
//   }
// //**************************************************************************************************** */
